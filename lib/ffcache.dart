...
Map<String, FFCacheSync> _ffcacheSyncs = {};

class FFCacheSync {
  /// Returns a FFCacheSync object.
  ///
  /// FFCacheSync objects are created and managed internally. FFCacheSync objects are creates only once for each [name].
  ///
  /// Cache files are stored in temporary_directory/[name] (default: ffcache).
  /// Cache entries expires after [defaultTimeout] (default: 1 day).
  /// ffcache uses _ffcache.json to store internal information. If you try to use '_ffcache.json' as key, it will through an Exception.
  static Future<FFCacheSync> getInstance(
      {String name, Duration defaultTimeout, bool debug}) async {
    name = name ?? _default_name;
    final cache = _ffcacheSyncs[name];
    if (cache != null) {
      if (debug != null) {
        cache._debug = debug;
      }
      ...
      debug = debug ?? false;
      defaultTimeout = defaultTimeout ?? _defaultTimeoutDuration;
      final newCache = FFCacheSync._(
          name: name, debug: debug, defaultTimeout: defaultTimeout);
      await newCache._init();
      _ffcacheSyncs[name] = newCache;
      return newCache;
    }
  }

  FFCacheSync._({String name, bool debug, Duration defaultTimeout}) {
    _name = name;
    _debug = debug;
    this.defaultTimeout = defaultTimeout;
  }
  ...
  Future<void> _init() async {
    ...
    print('_init_internal basePath=$_basePath');
  ...
  void setString(String key, String value) {
    setStringWithTimeout(key, value, defaultTimeout);
  }
  ...
  String getString(String key) {
    ...
    final filePath = _pathForKey(key);
  ...
  /// store (key, stringValue) pair. cache expires after timeout.
  void setStringWithTimeout(String key, String value, Duration timeout) {
    File(_pathForKey(key)).writeAsStringSync(value);
    _setTimeout(key, timeout);
  }

  void _setTimeout(String key, Duration timeout) {
    if (timeout.isNegative) {
    } else {
      _timeoutMap[key] = DateTime.now().add(timeout).millisecondsSinceEpoch;
  ...
  Future<void> _saveMap() async {
    ...
    _saveTimer = Timer(_save_map_after, () {
      String value = json.encode(_timeoutMap);
      File('$_basePath/$_ffcache_filename').writeAsStringSync(value);
      _saveTimer = null;
      if (_debug) {
        print('saved $_ffcache_filename');
      }
  ...
  ///
  /// jsonData is converted to string using json.encode and stored as (JSON) String.
  void setJSON(String key, dynamic data) {
    setJSONWithTimeout(key, data, defaultTimeout);
  }

  /// store (key, jsonData) pair. cache expires after timeout.
  ///
  /// jsonData is converted to string using json.encode and stored as (JSON) String.
  void setJSONWithTimeout(String key, dynamic data, Duration timeout) {
    String value = json.encode(data);
    File(_pathForKey(key)).writeAsStringSync(value);
    _setTimeout(key, timeout);
  }
  ...
  /// store (key, bytes) pair. cache expires after defaultTimeout.
  void setBytes(String key, List<int> bytes) {
    setBytesWithTimeout(key, bytes, defaultTimeout);
  }

  /// store (key, bytes) pair. cache expires after timeout.
  void setBytesWithTimeout(String key, List<int> bytes, Duration timeout) {
    File(_pathForKey(key)).writeAsBytesSync(bytes);
    _setTimeout(key, timeout);
  }
  ...
  bool has(String key) {
    ...
    return FileSystemEntity.typeSync(_pathForKey(key)) !=
        FileSystemEntityType.notFound;
  }

  /// remove all cache entries.
  void clear() {
    Directory(_basePath).deleteSync(recursive: true);
    Directory(_basePath).createSync(recursive: true);
